...
### Fixed
- Context-gated ID patterns (passport, driver's license, MRN, health card, German ID) no
  longer capture a plain word after the keyword ("passport office", "driver license
  renewal", "MRN record", "health card services"): the captured value must contain a digit
  (same fix as bank/insurance). Short all-caps acronyms (MRN, DEA, ...) are no longer read
  as names. Library and demo.
